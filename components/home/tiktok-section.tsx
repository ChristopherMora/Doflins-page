...
function TikTokEmbed({ videoId, label }: { videoId: string; label: string }): React.JSX.Element {
    ...
    <div ref={ref} className="relative aspect-[9/16] w-full overflow-hidden rounded-2xl bg-[var(--surface-100)]">
      {visible ? (
        ...
        <div className="flex h-full flex-col items-center justify-center gap-2">
          <div className="h-12 w-12 animate-pulse rounded-full bg-[var(--surface-200)]" />
          <span className="text-xs text-[var(--ink-400)]">{label}</span>
        </div>
      )}
    </div>
  );
    ...
    <section className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4 sm:gap-4">
        {TIKTOK_VIDEOS.map((video) => (
          <TikTokEmbed key={video.id} videoId={video.id} label={video.label} />
        ))}
      </div>
